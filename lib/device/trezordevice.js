...
class TrezorDeviceWrapper extends DeviceWrapper {
  ...
  async signTransaction(mtx, inputData) {
    ...
    const rings = await this.deriveRings(mtx, inputData);
  ...
  /**
   * TODO: Change input format, to include prevout info inside input data.
   * Grab all public keys for the rings.
   * @private
   */

  async collectPublicKeys(inputData) {
    const pks = new BufferMap();

    for (const [key, input] of inputData.entries()) {
      const pk = await this.getPublicKey(input.path);
      pks.set(key, pk);
    }
  ...
  /**
   * Derive rings to apply signatures.
   * @private
   */

  async deriveRings(mtx, inputData) {
    const pks = await this.collectPublicKeys(inputData);

    const rings = new BufferMap();

    for (const input of mtx.inputs) {
      const {prevout} = input;
      const poKey = prevout.toKey();
      const pk = pks.get(poKey);
      const data = inputData.get(poKey);

      assert(pk, 'Could not get the public key');
      assert(data, 'Could not get data for the input.');

      const witness = data.witness;
      const coin = data.coin;
      let nested = false;

      if (data.witness && coin.getType() === 'scripthash')
        nested = true;
...
//function getMultisigScript(multisig, network) {
//  const pubkeys = [];
//  for (const pk of multisig.pubkeys) {
